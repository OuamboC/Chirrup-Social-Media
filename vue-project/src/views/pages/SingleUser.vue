<template>
    ...
    <div class="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50">
        ...
        <div v-if="loading" class="flex flex-col items-center justify-center py-20">
            ...
            <p class="mt-4 text-gray-600 font-medium text-lg">Loading user profile...</p>
        </div>

        <!-- Error State -->
        <div v-if="error && !loading" class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div v-if="!loading && user.username">
                ...
                <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                    ...
                    <div class="flex flex-col md:flex-row items-center md:items-start space-y-4 md:space-y-0 md:space-x-8">
                        ...
                        <div class="flex-shrink-0">
                            <div class="w-32 h-32 bg-white rounded-full flex items-center justify-center shadow-2xl border-4 border-white">
                        ...
                        <div class="flex-1 text-center md:text-left">
                            ...
                            <!-- User Stats -->
                            <div class="flex justify-center md:justify-start space-x-8 mt-6 mb-6">
                                <div class="text-center">
                                    <div class="text-3xl font-bold text-white">{{ user.posts?.length || 0 }}</div>
                                    <div class="text-sm text-blue-100">Posts</div>
                                </div>
                                <div class="text-center">
                                    <div class="text-3xl font-bold text-white">{{ user.followers?.length || 0 }}</div>
                                    <div class="text-sm text-blue-100">Followers</div>
                                </div>
                                <div class="text-center">
                                    <div class="text-3xl font-bold text-white">{{ user.following?.length || 0 }}</div>
            ...
            <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div class="mb-6">
                    ...
                    <p class="text-gray-600">{{ user.posts?.length || 0 }} {{ user.posts?.length === 1 ? 'post' : 'posts' }}</p>
                </div>

                <!-- Success Message -->
                <div v-if="successMessage" class="mb-4 bg-green-50 border-l-4 border-green-500 rounded-lg p-4 shadow-md animate-fade-in">
                    <div class="flex items-center">
                    ...
                    >
                        <router-link :to="`/posts/${post.post_id}`" class="block p-6">
                            <div class="flex items-start space-x-4">
                                <!-- Avatar -->
                                <div class="flex-shrink-0">
                                    <div class="w-12 h-12 bg-gradient-to-br from-blue-500 to-green-500 rounded-full flex items-center justify-center text-white font-bold text-lg shadow-md">
                                        {{ getInitial(user.username) }}
                                    </div>
                                </div>
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center justify-between mb-2">
                                        ...
                                        <svg class="w-5 h-5 text-gray-400 group-hover:text-blue-600 transition-colors duration-200 transform group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <div class="flex items-center space-x-4 text-sm text-gray-500">
                                        <span class="flex items-center hover:text-blue-600 transition-colors duration-200">
                                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                            </svg>
                                            {{ formatDate(post.timestamp) }}
                                        </span>
                                        <span class="flex items-center hover:text-red-600 transition-colors duration-200">
                                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <!-- Empty State -->
                <div v-if="!user.posts || user.posts.length === 0" class="text-center py-16">
                    <div class="bg-white rounded-xl shadow-md p-12 border border-gray-100">
                        ...
                        <h3 class="text-2xl font-semibold text-gray-700 mb-2">No posts yet</h3>
                        <p class="text-gray-500 text-lg">{{ user.first_name }} hasn't shared anything yet.</p>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    computed: {
        ...
        isFollowing() {
            ...
            return this.user.followers.some(follower => follower.user_id == this.currentUserId);
        }
    },
    methods: {
        loadUserData() {
