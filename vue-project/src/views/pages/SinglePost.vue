<template>
  <!-- Main Content -->
  <div
    class="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50 py-8"
  >
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div
        v-if="error"
        class="bg-red-50 border-l-4 border-red-500 rounded-lg p-6 shadow-md"
      >
      ...
      >
        ...
        <div class="p-8">
          ...
          <!-- Post Meta -->
          <div
            class="flex items-center space-x-6 text-sm text-gray-500 pb-6 border-b border-gray-200"
          >
            ...
            <span class="flex items-center">
              <svg
                ...
                :class="{ 'fill-current': userLikedPost }"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
      ...
      <div
        v-if="successMessage"
        class="mt-4 bg-green-50 border-l-4 border-green-500 rounded-lg p-4 shadow-md animate-fade-in"
      >
        <div class="flex items-center">
          <svg
...
export default {
  ...
  methods: {
    likePost() {
      authPostService
        .likePosts(this.$route.params.id)
        .then(() => {
    ...
    unlikePost() {
      authPostService
        .unlikePosts(this.$route.params.id)
        .then(() => {
    ...
    DeletePost() {
      authPostService
        .deletePosts(this.$route.params.id)
        .then(() => {
        ...
        .catch((error) => {
          this.error =
            error ||
            "Failed to delete the post. You can only delete your own posts.";
        });
    },
    EditPost() {
